def get_human_age(cat_age: int, dog_age: int) -> list:
    ...
    if cat_age > 100 or dog_age > 100:
        raise ValueError("Ages cannot be greater than 100.")

    # Calculate cat's human age
    if cat_age <= 15:
        cat_human_age = cat_age // 15
    elif cat_age <= 24:
        cat_human_age = 1 + (cat_age - 15) // 9
    else:
        cat_human_age = 2 + (cat_age - 24) // 4

    # Calculate dog's human age
    if dog_age <= 15:
        dog_human_age = dog_age // 15
    elif dog_age <= 24:
        dog_human_age = 1 + (dog_age - 15) // 9
    else:
